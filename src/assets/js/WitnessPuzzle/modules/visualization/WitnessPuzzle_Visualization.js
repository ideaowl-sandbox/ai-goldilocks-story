...
class WitnessPuzzle_Visualization {
        ...
        if (svg.selectAll('.foregroundRect').empty()) {
            svg.append('rect')
                ...
                .on('keydown', function (ev) {
                    self.__userKeyDown(ev, self._core);
                })
                .on('click', function () {
                    if (self._core._state.mode !== 'image' 
    ...
    __userKeyDown(d3Event, core) {
        ...
        if (d3Event.keyCode === KEYCODE_W
            || d3Event.keyCode === KEYCODE_UP) {
            userMove[1] = 1;
            keyDirection = 'u';
        }

        if (d3Event.keyCode === KEYCODE_D
            || d3Event.keyCode === KEYCODE_RIGHT) {
            userMove[0] = 1;
            keyDirection = 'r';
        }

        if (d3Event.keyCode === KEYCODE_S
            || d3Event.keyCode === KEYCODE_DOWN) {
            userMove[1] = -1;
            keyDirection = 'd';
        }

        if (d3Event.keyCode === KEYCODE_A
            || d3Event.keyCode === KEYCODE_LEFT) {
            userMove[0] = -1;
            keyDirection = 'l';
        }
